...
def sortField = "Plan Go-live Date"
// 遠端 URL 及驗證資訊
...
String getJsonSafeString(Issue issue, String cfName) {
    ...
    if (rawValue instanceof java.util.Map) {
        def stringEntries = rawValue.collect {
            key, value ->
...
def getSafeUsername(Issue issue, String cfName) {
    ...
    if (userObject instanceof java.util.Collection) {
        return userObject.collect { user -> user?.getDisplayName() }.join(', ')
    }
...
/* To-do: 卡控條件更改： 
    改爲撈取 Plan go-live date 前一周，且需要符合條件：
        最後一次切換至golive approval必須早於 plan go-golive date

    思考一下當場景可能會有哪些，髒數據、重傳的問題
*/
...
def jqlQuery = "project = '${projectKey}' AND status = '${targetStatus}' ORDER BY '${sortField}' DESC"
log.warn("Executing JQL: ${jqlQuery}")

def issues = Issues.search(jqlQuery).issues
...
def processedIssues = issues.collect { issue ->
    [
        issue: issue,
        planGoliveDate: getSafeCfValue(issue, "Plan Go-live Date") as LocalDateTime,
        transitionDateTime: getLastTransitionDateToStatus(issue, targetStatus)
    ]
}.findAll {
    it.transitionDateTime &&
    getSafeCfValue(it.issue, "Show on Go-live") != "No" &&
    it.planGoliveDate && // Ensure planGoliveDate is not null
    !it.transitionDateTime.isAfter(it.planGoliveDate) &&
    it.transitionDateTime.isAfter(startTime)
}


log.warn("精準過濾後，找到 ${processedIssues.size()} 個符合條件的議題。 ")
...
def issuesByReviewer = processedIssues.groupBy {
    getSafeCfValue(it.issue, projectReviewerCfName) as ApplicationUser
}
...
issuesByReviewer.each {
    ...
    def detailsList = issuesInGroup.collect { item ->
        def issue = item.issue
        def planGoliveDate = item.planGoliveDate
        ...
        return  [
            ...
            "Show_golive"                : "Yes"/* getJsonSafeString(issue, "Show on Go-live") */,
            "Announcement_plan"          : getJsonSafeString(issue, "Announcement Plan"),
            "plan_golive_date"           : planGoliveDate ? planGoliveDate.format(dateOnlyFormatter) : null,
            "itpm"                       : getSafeUsername(issue, "ITPM"),    // 使用ID
            "po"                         : getSafeUsername(issue, "Major Product Owner"),      // 使用ID
            "Is_routinely go-live"       : getJsonSafeString(issue, "Is Routinely go-live"),
            "System_dev_lead"            : getSafeUsername(issue, "System Dev Lead"),         // 使用ID
            "Deployment_owner"           : getSafeUsername(issue, "Deployment Owners"),        // 使用ID
            "Development_Team"           : getJsonSafeString(issue, "Development Team"),    
            "apply_golive_request_date"  : now.format(dateTimeFormatter),//{ def d = getSafeCfValue(issue, "Apply Go-live Request Date"); d ? dateTimeFormat.format(d) : null }(),
            "Reason"                     : getJsonSafeString(issue, "Reason")
        ]
    }
    
    if(!detailsList) return //  批次内沒有符合條件的單子，提前跳過這次iter
