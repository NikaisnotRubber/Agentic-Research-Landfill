...
issues = issues.findAll {
    log.warn("type is: ${it.getClass()}")
    Date transitionDate = getLastTransitionDateToStatus(it, targetStatus)
    LocalDateTime planGoliveDate = getSafeCfValue(it, "Plan Go-live Date") as LocalDateTime

    if (transitionDate) {
        if(getSafeCfValue(it, "Show on Go-live") == "No") return false
        ...
        // 如果轉換時間在我們設定的 startTime 之後且在 plan_golive_date 之前，就保留這筆 issue
        if(transitionDateTime.isAfter(planGoliveDate))    return false
...
issuesByReviewer.each {
    ...
    def detailsList = issuesInGroup.collect {   issue ->
        def planGoliveDate = getSafeCfValue(issue, "Plan Go-live Date") as LocalDateTime 
