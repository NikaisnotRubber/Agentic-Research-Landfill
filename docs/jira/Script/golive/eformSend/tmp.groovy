...
def getSafeUserID(Issue issue, String cfName) {
    ...
    if (userObject instanceof java.util.Collection) {
        return userObject.collect { ApplicationUser user -> user?.getId() }.join(', ')
    }
...
issuesByReviewer.each {
    ...
    def detailsList = issuesInGroup.collect {   issue ->
        [
            ...
            "IT_Platform"                : getSafeValFromMap(issue, "IT Platform & Module", 0),
            "moduleString"               : getSafeValFromMap(issue, "IT Platform & Module", 1),
    ...
    try {
        ...
        http.request(POST, JSON) {
            ...
            response.success = { resp, json ->
                ...
                if (qaNo) {
                    String formUrl = "https://dgdvap.deltaww.com/SmartFormV3/#/?FlowCode=2890&InstanceCode=${qaNo}&ToCode=2"
                    ...
                    if (urlCfList.isEmpty()) {
                        log.error("找不到名為 'Request Go-Live Approval' 的自定義欄位，無法更新議題。")
                    } else {
                        def urlCustomField = urlCfList.first()
                        // 獲取 IssueManager 以便在迴圈中拿到可寫的 MutableIssue
                        def issueManager = ComponentAccessor.getIssueManager()

                        issuesInGroup.each { issueFromLoop ->
                            try {
                                // 最佳實踐：根據唯讀的 issueFromLoop ID，獲取一個保證可寫的 MutableIssue 物件
                                def mutableIssue = issueManager.getIssueObject(issueFromLoop.getId())
                                ...
                                if (mutableIssue) {
                                    ...
                                    if (finalValue == null || finalValue != formUrl) {
                                        log.error(">>> 警告：欄位值未能成功寫入議題 ${mutableIssue.key}！請檢查是否有其他腳本或監聽器正在干擾，或檢查字段是否被鎖定。")
                                    } else {
                                        log.error("無法獲取議題 ${issueFromLoop.key} (ID: ${issueFromLoop.getId()}) 的 MutableIssue 物件，可能已被刪除或權限不足。")
                                    }
                                }
                            } catch (Exception fieldUpdateEx) {
                                log.error("更新議題 ${issueFromLoop.key} 的 'Request Go-Live Approval' 欄位時發生例外: ${fieldUpdateEx.message}", fieldUpdateEx)
                            }
                        }
                    }
                } else {
